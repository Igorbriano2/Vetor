# 09 — Plano de migração para a visão JARVIS (Manus)
...
| Design tokens na direção do ADR-001 (grafite/ciano/âmbar/coral) | 🔄 em andamento nesta rodada |
| Schema de missões (`missions`, `mission_steps`, `agent_runs`, `approvals`) | 🔄 em andamento nesta rodada, aditivo |
...
## Fase 2 — JARVIS funcional

*Equivalente Manus: Fase 2 ("Orquestrador, agente geral, fila, eventos, timeline e aprovações").*

| Item | Status |
|---|---|
| Prompt oficial do JARVIS (agente geral) | 🔄 em andamento nesta rodada — adota `system-jarvis.md` do Manus |
...
|---|---|---|
| Agente Geral | JARVIS | Mesmo agente, prompt substituído pelo `system-jarvis.md`, mais rigoroso (schemas estruturados, separação fato/inferência/recomendação) |
