...
| F | Integração com Videomaker/Planejamento (Fase 6) — preservar timeline/estágios já provados | D, E | **Feita (escopo mínimo, sem tocar no pipeline de vídeo)** — `GerarPecasCampanha.tsx` agora roteia cada peça do calendário pro agente certo: itens cujo `tipo` indica vídeo (`ehConteudoDeVideo` — "vídeo"/"video"/"reels") viram etapa do agente `video` (`criar_briefing`, baixo risco) em vez de `design`; a etapa de copy passa a depender da etapa visual certa (design OU vídeo) da mesma peça. Testado ao vivo com o calendário real da Dog King Cambé (item "Vídeo preparação dog na chapa", tipo "Vídeo 20-25seg"): antes do deploy desta rodada a etapa saiu como `design` (comportamento da Rodada E, ainda sem o roteamento); depois do deploy, nova geração confirmou via SQL direto que a mesma peça agora sai como `agente=video, ferramentas=[criar_briefing]`, as outras 4 continuam `design`. Além disso, `/design` e `/videomaker` agora mostram "campanha: {título da missão}" nos cards de "Projetos editáveis" (`design_projects`/`video_projects`) quando `mission_id` está setado — mesmo padrão de link já usado em `ArtifactLibrary.tsx` pras Entregas. Nenhuma mudança em `videoPipeline.ts`/`designProjects.ts`/estágios do pipeline — só roteamento de missão + link de UI. |
| G | ProviderRouter por critério + créditos (Fase 7) | — (pode rodar em paralelo às demais) |
| H | Testes E2E, segurança, performance, deploy (Fase 8) | todas | **Parcial** — suíte automatizada completa passando (`apps/agentes`: 233/233, `apps/painel`: 53/53), `get_advisors` (security) sem achados novos além dos 3 pré-existentes (SECURITY DEFINER de `current_cliente_id`/`current_papel`, leaked-password-protection desligado — nenhum introduzido por esta rodada). E2E real em produção: geração em lote (Rodada E/F) criou uma missão real de 10 etapas a partir do calendário real da Dog King Cambé — 9 etapas ficaram `pending/ready/running` corretamente, **1 falhou** (`design`, "nenhum artefato verificável foi produzido, o especialista não retornou um resumo") — achado real, não é regressão desta rodada: é a mesma flakiness ocasional do especialista de Design já registrada na seção 5 deste documento ("mais estrutura tende a exigir mais turnos de LLM"); o fix de `checarConclusaoMissao` desta sessão trata isso corretamente (a etapa falha não trava a missão pra sempre). Segurança revisada manualmente pro código novo desta sessão (`GerarPecasCampanha.tsx`, `/design`, `/videomaker`): nenhuma confiança em dado vindo do cliente pra cliente_id/risco (reusa `/api/missoes` existente, que já resolve cliente_id da sessão e recalcula risco no servidor), sem XSS (nenhum `dangerouslySetInnerHTML`), RLS já cobre os embedded joins (`missions(titulo)`, mesmo padrão de `fetchArtifacts.ts`). **Achado real e crítico durante o E2E** (mesma missão real acima): checando `agent_runs.custo_estimado_centavos` pras etapas de `design`, 3 das 5 tinham custo real (26¢, 29¢, 11¢) e o artefato gerado era `type=image, title="Peça de design (camadas editáveis)"` — ou seja, `criar_peca_de_design` (ferramenta de geração de imagem PAGA de verdade) rodou automaticamente em etapas declaradas só com `criar_briefing`/`gerar_design` (baixo risco, sem aprovação), quebrando a garantia "zero custo automático" documentada na Rodada E. Causa raiz: `FERRAMENTA_GERACAO_POR_AGENTE[agenteId]` em `specialistRunner.ts` é indexado só por agente, nunca filtrado pelo que a etapa de fato declarou em `mission_steps.ferramentas` — então QUALQUER etapa de Design/Vídeo sempre tinha acesso irrestrito às ferramentas de geração real do agente, independente do risco calculado/aprovado pelo Policy Engine. Agravado por dois problemas conexos: `criar_peca_de_design` (sucessor de `gerar_imagem`, "use SEMPRE... nunca gerar_imagem") nunca esteve registrada em `tools/registry.ts` (ficaria fail-closed crítica se algum dia fosse checada, mas nunca era); e `editar_video_timeline`/`finalizar_video_com_legendas`/`analisar_video_de_referencia` (pipeline de Vídeo já provado) também nunca estiveram registradas, apesar de serem só compute local (ffmpeg), sem custo externo. **Corrigido**: `criar_peca_de_design` registrada como `medium` (mesmo risco de `gerar_imagem`, do qual é sucessora — com alias `ALIAS_FERRAMENTA_GERACAO` em `specialistRunner.ts` pra uma etapa aprovada com `gerar_imagem` continuar liberando a ferramenta real que o especialista de fato chama); as 3 ferramentas do pipeline de Vídeo registradas como `low` (preserva exatamente o comportamento atual, já provado, sem exigir aprovação nova); e `ferramentasGeracao` em `specialistRunner.ts` agora é filtrada — só libera uma ferramenta de geração real se ela (ou seu alias) foi declarada na etapa, ou se é `low` no registro. Suíte completa (233 testes) e build seguem passando sem alteração após o fix — nenhuma etapa de teste cobre este caminho especificamente (specialistRunner.ts não tem testes unitários hoje, roda contra Anthropic/Supabase reais), validação foi manual + E2E real em produção: gerada uma NOVA missão real (`2fb1473f-c0f3-4dc3-b099-f5923c8ad7ac`) depois do deploy do fix — a etapa de Design que completou saiu com `artifact_type=document` ("Briefing de Design...", `custo_estimado_centavos=23` — só o custo normal da chamada Claude, mesmo de qualquer etapa) em vez de `artifact_type=image` como antes do fix. Confirmado: nenhuma geração de imagem paga disparou. **Faltando pra fechar Fase 8 de verdade**: testes E2E automatizados (hoje só manuais via browser), teste de carga/performance, revisão de segurança formal via skill dedicada (bloqueada neste ambiente — ver nota no início da sessão), teste unitário pro filtro de `ferramentasGeracao` (specialistRunner.ts ainda não tem suíte). |
