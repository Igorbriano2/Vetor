# ADR-001 — Direção futurista e JARVIS como experiência central
...
O frontend precisa de um design system próprio, componente de estado do JARVIS, eventos em tempo real e uma hierarquia baseada em missões. O backend precisa expor estados de execução, aprovações e eventos. A implementação será mais cuidadosa que um CRUD, mas a diferenciação será percebida em todo o produto.
