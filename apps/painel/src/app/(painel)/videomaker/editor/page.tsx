...
// Cria um video_project em branco e redireciona pro editor de verdade
// (/videomaker/editor/[projectId]). Normalmente um projeto nasce de um
// pedido ao Vetor com múltiplos arquivos (Parte 2/4, ainda não conectado
// — task #79); isto aqui cobre o caminho manual "começar do zero direto
// no painel", mesmo padrão já usado no Design (design/editor/page.tsx).

import { useState } from "react";
import { useRouter } from "next/navigation";
import { createSupabaseBrowserClient } from "@/lib/supabase/client";
import { criarTimelineVazia } from "@/lib/video/timelineTypes";

const FORMATOS = [
  { label: "Feed/Reels vertical (1080×1920)", width: 1080, height: 1920 },
  { label: "Feed quadrado (1080×1080)", width: 1080, height: 1080 },
  { label: "Paisagem (1920×1080)", width: 1920, height: 1080 },
];

export default function NovoVideoProjectPage() {
  const router = useRouter();
  const [criando, setCriando] = useState(false);
  const [erro, setErro] = useState<string | null>(null);
  const [titulo, setTitulo] = useState("Novo vídeo");
  ...
  async function criar() {
    ...
    const fps = 30;
    const timeline = criarTimelineVazia({ fps, width: formato.width, height: formato.height });

    const { data, error } = await supabase
      .from("video_projects")
      .insert({
        cliente_id: usuario.cliente_id,
        title: titulo.trim() || "Novo vídeo",
        width: formato.width,
    ...
    router.push(`/videomaker/editor/${data.id as string}`);
  }
  ...
  return (
    <div className="px-6 py-10">
      <div className="mx-auto max-w-md">
        ...
        <h1 className="mt-1 text-2xl font-bold text-areia">Novo vídeo</h1>
        <p className="mt-2 text-sm text-areia/60">
          Começa em branco no editor — a maioria dos vídeos nasce de um pedido ao Vetor no chat, isto é pra
          começar direto no painel.
        </p>
