...
import { gerarPerfilVisualDeReferencia } from "../negocio/referenceImageAnalysis.js";
...
// Fase 2 do reset de produto (docs/PRODUCT-RESET-AUDIT.md) — "Ver análise
// visual" pra referências de IMAGEM, generalizando o mesmo padrão acima.
referenciasRouter.post("/:id/analisar-imagem", async (req, res) => {
  const { id } = req.params;
  const { cliente_id } = req.body ?? {};
  if (!cliente_id) {
    res.status(400).json({ error: "cliente_id é obrigatório" });
  ...
  try {
    const perfil = await gerarPerfilVisualDeReferencia({
      clienteId: cliente_id,
      assetId: item.asset_id as string,
      referenceLibraryItemId: item.id as string,
    });
    ...
    console.error(`Erro ao analisar referência (imagem) ${id}:`, err);
    res.status(500).json({ error: err instanceof Error ? err.message : "Falha ao analisar a imagem de referência." });
  }
});
